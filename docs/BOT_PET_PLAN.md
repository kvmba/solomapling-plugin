...
测试：`BotPetAssignerTest`（10 例，纯策略）+ `BotPetConfigTest`（2 例，YAML 键位 + 池加载）+ `BotPetFactoryTest`（3 例，无 DB 反射造宠）+ `PetCommandInterpreterTest`（5 例，行为选择）。全量 `mvn test` 840 项通过。
