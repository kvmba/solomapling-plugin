...
- `BotPet.yaml`（`PluginResources` 解析）：
  ...
  speak:   { chance: 0.25, min_interval_ms: 8000, max_interval_ms: 25000 }
...
11. **宠物行为/说话（WZ 驱动）**：每种宠物在 `Item.wz/Pet/<id>.img/interact` 有自己的行为菜单——每条行为的 `prob`（=**服从**触发%）、等级段 `l0`–`l1`、以及 `success`/`fail` 两套 `act`（动作）。台词在 `String.wz/PetDialog.img/<id>/<key>`（**有 zh-CN 本地化**）。插件按 petLevel 过滤可用行为、等概率抽一条，再按 `prob` 掷"服从/不服从"，用宿主 `PacketCreator.commandResponse(cid, idx, talk=!obey, commandIndex, balloon)` 触发；**动画与台词都由客户端从本地 WZ 解析**——与宿主自己的 `PetCommandHandler` 同机制。节奏：每 pet 冷却 `speak.min/max_interval_ms`，每次触发概率 `speak.chance`（默认 0.25）。**与宿主 `PetCommandHandler` 完全同一条路径**，因此 bot 宠物在被"触发"时的表现与真人指令宠物一致；`hasPetChatballoon` 只影响气泡装饰（v83 的宠物名牌/Quote Ring 数据有限，随宠物装备槽如实上报）。

**审核修正（本轮）**：移除早期版本的多余地图变化重定位（宿主已处理，避免重复 showPet）；follower 改为只遍历"有宠物的 bot"（`TRACKED` 集合）并按 `SoloMaplingUtilities.getChr` 解析（授予发生在 BotSM 包装之前）；`grantForBot` 对"已有宠物"的幂等分支补 `track`（companion 重载后仍能跟随）；`createPersistent` 在 CASH 满时清理孤儿 `pets` 行；`BotPetPool` 兼容 yamlbeans 把裸数字读成字符串。**第二轮复审**：`BotPetFollower.start` 增加 `rescan()`——否则 `!botpet reload` 清空 `TRACKED` 后，已有宠物的 bot 不再被跟随；`removePets` 增加无宠快速返回；`!botpet clear` 回显真实宠物数与 companion 说明。**第三轮完整复审**：修复 **宠物名 NPE**——`ByteBufOutPacket.writeString` 对 null 无保护，而 `addPetInfo` 会写 `pet.getName()`；未命名宠物原先 `name==null`，会让 `showPet` / `spawnPlayerMapObject` NPE（整个授予失败）。现 `BotPetFactory` 保证名字**永不为 null**（未命名用占位名）；清理死代码：`BotPetAssigner` 去掉无用的 `names` 参数与 `BotPetNamesProvider` 接口、`BotPetPool` 去掉未用的 `drawDistinct`、`hasAnyPet` 降为私有、删除未使用的 `playerNearbyRange` 配置。**第四轮完整复审**：`createPersistent` 在 `loadFromDb` 失败时清理刚建的 `pets` 行（孤儿）；follower 改为**逐 bot 隔离异常**（对齐 `GrindTickRegistry` 的 per-participant 约定：单个 bot 抛错不再中断整轮 tick）；`!botpet` 帮助文本与实现对齐。**第五轮完整复审**：修正**宠物槽位（slot）语义**——`MOVE_PET` / `PET_COMMAND` / 装备槽 / 名签槽都以**宠物数组下标**为键（宿主从 `getPetIndex` 取），此前 follow 循环用"跳过 null 不加一"的计数器，一旦 `pets[]` 出现空洞（宿主 `SpawnPetProcessor` 在无多宠技能时调 `unEquipPet(pet0, false)` 会留下 `[0]=null, [1]=pet`）就会把后排宠物错位到 0 号槽；现改为按**数组下标**遍历（与 `loot()` 一致），授予时用 `getPetIndex(pet)` 读取引擎实际分配的槽位（对齐 `SpawnPetProcessor`），不再假设计数器。**第六轮完整复审**：修复**宠物装备不刷新角色外观**——宠物装备（名牌/拾物/磁铁）随**角色外观包**下发（body part 14/21/22/23，见 `addCharEquips`），观察者只有经 `equipChanged()` 广播才会收到；宿主的 `equip/unequip` 路径都以 `equipChanged()` 收尾，而 `BotPetGear.equip` / `clearPetGear` 原先只改物品不改外观，导致名牌/装备在别人客户端不显示。现授予后与清除后各补一次 `equipChanged()`（每次授予只调一次，不是每只宠物一次）。**第七轮完整复审**：`equipChanged()` 会向 `getMap()` 广播且**不判空**，而 `Character` 初始 `map==null`（`getMap()` 在放置前为空），故外观刷新统一走 `refreshLook(bot)`——map 为空时跳过广播，避免在地图未就绪的 bot 上 NPE。**第八轮（两个可见 bug）**：①**原地踏步**——宠物姿态**不是角色姿态**。官方宠物姿态字节 = (动作序号<<1)|朝向，序号顺序 STAND,MOVE,JUMP,ALERT,PRONE,FLY,HANG，即 STAND=0/1、MOVE=2/3、FLY=10/11。此前站立误用角色 `STAND_*=4/5`（对宠物是非法动作→客户端循环上一个动画=原地走），走路用的 2/3 恰好是宠物 MOVE 所以正常。两处独立事实印证：宿主 `SpawnPetProcessor` 召唤用 `setStance(0)` 且宠物静止站立（→0=STAND），旧代码走 2/3 显示正常（→2/3=MOVE）。**明确不采信 maplestory-wasm/Journey**（它把前两个反了：MOVE=0/1、STAND=2/3，与官方不符）。**（该轮结论有误，见第九轮订正）**。②**进图闪现**——follower 原在 `!isMapObserved` 时整体 `return`，宠物坐标在无人时**冻结**（而 bot 仍按 coarse 移动）；玩家进图的 `spawnPlayerMapObject` 带的是陈旧宠物坐标，下一 tick 才发 `MOVE_PET` 拉回→闪现。现改为**每 tick 都同步宠物坐标**（纯内存），仅把**广播/说话/拾取**留给被观察的地图；陈旧度因此也被限制在一个 tick 内。**第九轮（依据真实客户端 IDA 订正，权威）**：在完整命名的 GMS095 客户端（与 083 同源）里反编译 `CPet::OnResolveMoveAction`(0x6a06c0) 与 `CPet::MoveAction2RawAction`(0x6a0ff0)，得到**权威**的宠物 move-action 字节——`moveaction>>1`=动作序号、`moveaction&1`=朝向，且 **0 落到默认分支→序号 0 = MOVE（不是站立！）**：MOVE=2/3、STAND=4/5、JUMP(空中)=6/7、SWIM(水)=12/13、HANG(梯绳)=30/31；宠物**没有独立 FLY 动作**。因此上轮把 STAND 改为 0/1 反而制造了原地踏步（0=MOVE）；正确 STAND=4/5。**同时按真实 CPet 行为重写跟随**：①朝目标点（bot 身后小错位）以固定速度**滑动**逼近，**不再瞬移到位**（瞬移正是 bot 转身时宠物"闪到身后"的原因），仅在距离 > `teleport_dist_px`（如传送后）才瞬移；②每步把 y **吸附到自身 x 下方的地面**（`findBelow`+`calculateFooting`，向上探测若干像素），消除"悬空走路"；③水中用 SWIM=12/13 且 `fh=0` 漂浮；④bot 在绳/梯时宠物用 HANG=30/31 挂在其身侧；⑤bot **腾空**（跳跃或高空坠落）时宠物也跟着腾空、保持主人高度并用 JUMP=6/7（腾空时不贴地，`noGravity`）。**拾取**：新增每宠冷却 `pickup.cooldown_ms`（默认 **1000ms**）且 `max_per_tick=1`；且**只拾取怪物掉落的道具**（`MapItem.getDropper() instanceof Monster`，玩家投掷的 `dropper` 是 Character 会被跳过）。**第十轮（实机反馈）**：①**宠物太贴主人/多宠未散开**——原目标 x = `bot.x + index*22`，第 0 只正好落在主人身上；改为**对称扇形散开** `spreadFor(index)`（0→+45、1→−45、2→+90），水陆一致。②**水下踏步**——权威反编译 `CPet::OnResolveMoveAction` 确认游泳 move-action 就是 12/13，值没错；真因是**死区分支重发姿态时沿用了宠物自身的旧 fh**（陆地 foothold），宠物进泳图后常处于死区/短距离，于是带着**非 0 的陆地 fh** 被客户端锚到海床 → 渲染成走路。改为**漂浮态（水/空/绳，`noGravity`）时 fh 一律清 0**；另把水判据放宽为 `map.isSwim() || CharacterStance.isSwimming(bot)` 以覆盖 bot 已在水中游的情况。**第十一轮（完整物理 + 官方跟随）**：宠物改为**像 bot 一样拥有完整的速度积分物理**（水平加速/摩擦、重力、落地、泳姿浮沉），不再每 tick 把位置 lerp 到目标：①**过远即传送**——官方行为是宠物落后太多时**直接闪现到主人身边**再继续跟随（**不是**加速冲刺），陆地按 x 间距、水中按 2D 距离超 `teleport_dist_px` 即回位；②**重力只向下**——主人跳跃/高空坠落时宠物**不会**被拉上天（删除了跟随跳跃的 hop 与升空分支），只有宠物自身脚下无地时才下落；③**泳图浮沉**——水中施加下坠重力（590）+ 低于主人上方目标点时的浮力（900），形成“下坠再浮起”的摆动，fh=0；④**随机间距**——每只宠物取一次稳定的随机偏移（±60px，彼此独立，允许部分重叠），不再等距扇形。**性能**不变：无真人观测时零 foothold 查询、零发包。**第十三轮（命名与属性）**：①**命名池换为中文昵称**（`BotPetNames.yaml`，98 个国人常用宠物名：团团/豆豆/糯米/旺财/小汤圆…），**命名概率 25%**；②**未命名的宠物走官方默认名**——取 `String.wz/Pet.img` 的**本地化道具名**（zh-CN 下即“褐色小猫”“粉红兔子”等），与真人新宠物一致（`BotHelpers.itemNameOrNull` 的安全包装；WZ 缺失时回退占位名，保证包序列化不 NPE）；③**随机属性**：饱满度 `60..100`、亲密度 `0..300` 且**参考 bot 等级/实力**（`0.7×等级 + 0.3×实力` 为中点、±60 抖动，低等级中位≈39、高等级≈291）。**注意**：bot 宠物不注册饥饿衰减（宿主 `registerPetHunger` 会 `saveToDb`，且只由客户端包驱动），故这些值一次定下后不会漂移；亲密度虽可达 300（宠物 10 级阈值 434），但**升级只由客户端包触发**，bot 无客户端，因此不会自行升级。**第十二轮（LOD 对齐 bot + 官方传送实现）**：①**LOD 与 bot 自身移动一致**——无真人观测时**整只宠物 tick 直接跳过**（不做物理/查询/发包），玩家进图的**首个 tick** 才跑一次完整物理做就地追平/下落/回位（即官方的「进图即刷新」做法），不再每 tick 空转；②**官方传送实现**——宠物「过远回位」不再是直接发 `MOVE_PET` 闪现，而是按官方序列 **先发移除宠物包（`showPet(remove=true)`）→ 更新坐标 → 再发召唤宠物包（`showPet`）**；③**过远判定改为 2D 距离**——原先只看水平 x，导致主人竖直下坠时宠物不跟下去，现按到主人的 2D 距离判定，主人坠落到下方也会把宠物回位到其身边。

**第十四轮（跟随改为单向皮带 + 补宠物说话）**：①**"bot 转身宠物就动"** —— 旧跟随目标 = `ownerX - facing * dist`，沿**主人朝向**取点，所以主人原地转身时目标从 `ownerX-dist` 跳到 `ownerX+dist`（位移 2×dist，远超 30px 死区），宠物必然绕到另一侧；且**主人朝宠物走**时宠物会主动后退（目标被推到身后）。改为**单向皮带模型**（`followTargetX(ownerX, petX, comfort)`）：宠物只在自己到主人的距离**超过** comfort 时才靠近，且只走到**自己这一侧**的 comfort 环上（`ownerX ± comfort`），绝不越过主人、绝不主动拉开距离。于是主人原地转身/小步移动/朝宠物走来都**不动**宠物；只有主人走远到超过 comfort 才追。②**舒适距离固定** —— 删除"主人停下的瞬间重掷跟随距离"（`lastOwnerMovingByBot` + `rerollFollowDistances`），每只宠物只掷一次 comfort 距离并**终身保持**，静止时就是老实站着。③**补宠物 chat** —— `maybeSpeak` 原先只发 `PET_COMMAND`（`commandResponse`，**包里没有任何文字**），客户端（Journey/OpenStory/MortalClient 的 `PetCommandHandler` 均如此）只播 `interact` 的 act 动画、不显示台词，故表现为"只有动作、没有话"。现除动作包外**再发一条 `PET_CHAT`**（`PacketCreator.petChat`，与宿主转达真人 `/pet` 同路径），文案从新类 **`PetDialogTable`** 读 `String.wz/PetDialog.img/<id>`（**本地化，zh-CN 即中文**），按本次命令序号取 `c<N+1>_s*`（服从）/ `c<N+1>_f*`（拒绝）语料，与动作同源。`PetChatHandler` 的 act 约束 [0,9]，用 `PET_CHAT_ACT = 0`。

测试：`BotPetAssignerTest`（10 例，纯策略）+ `BotPetConfigTest`（2 例，YAML 键位 + 池加载）+ `BotPetFactoryTest`（4 例，无 DB 反射造宠 + 非空名）+ `PetCommandInterpreterTest`（5 例，行为选择）+ `PetLeashAndChatTest`（7 例，单向皮带 + 台词选择）。全量 `mvn test` 1030 项通过。

**第十五轮（下落物理）**：实机反馈——**bot 跳下后宠物像被"拉扯"下去**，而不是宠物自己跳下去。根因：`followLand` 的下坠分支用 `simulateAirStep(..., ay=0)` **从静止自由落体**，且落地/空中都对外发 `vy=0`，观察者只看到 200ms 一次、竖向速度为零的位置跳变——渲染成"滑动/被拽"，而非坠落。（核查订正：`JUMP_REACH_PX=160` 的 warp 只是**向上单向**——owner 在下方并不触发 warp，故此前"下落被 warp"的说法有误；这一版不改 warp 阈值。）修复：①owner 落到更低平台（`ownerBelowAndNotWalkingDown`，即宠物站在 bot 已离开的平台上）时，宠物改为**向上起跳的"下落跳"**（沿用原落点/列的水平速度 `vx`≈0，追加 `DROP_HOP_UP_PXS=-120` 的上抛），落地前带真实 `vy`，于是下落呈抛物线、且用宠物**自身的 JUMP 姿态**（`PET_JUMP_*`）——看上去是"宠物自己往下跳"。**刻意保持在宠物本列**（不瞄 owner 落点）：若朝 owner 前移会读成"被拽过去"，还可能越过较窄的下层平台。②`followLand` 落地/下落统一**对外发真实 `vy`**（`vyAir` 则取 `fallVy`，否则 0），使任何起跳/下落都渲染成抛物线而非滑动。**踩空走下平台**（`walkLost`）保持原逻辑（保留走出速度、`launchVy=0` 从边缘开始下落），行为与修复前一致。③**未改动** `forbidFallDown` 平台分支（仍一律 warp）与 warp 阈值——本轮先前的临时改动经复核**已回退**：`forbidFallDown` 的 warp 是刻意的（该类平台不可穿落，回归用例见 `4c165f7`），向其加"下落跳"会引入漏落 bug。新增 `BotPetDropPhysicsTest`（2 例）钉住起跳必上抛、顶点可见且一 tick 内已快速下坠。全量 `mvn test` 1058 项通过。

**第十六轮（宠物特效：传送 / 挂背）**：实机反馈——宠物"直接平移"、且官方还有可设置的宠物特效（如传送特效）尚未实现。核查结论：插件宠物系统此前**没有任何宠物特效发包**（`Teleport`/`HangOnBack`/`LevelUp`/`Evolution` 全缺），宿主 `PacketCreator.showPetLevelUp` 也只写死 `nType=0` 且插件从未调用。权威依据：GMS `PetEffectType` 枚举 `LevelUp=0 / Teleport=1 / HangOnBack=2 / Evolution=3`，资源在 `Effect.wz/PetEff.img`（`Basic/{LevelUp,Evolution,Teleport,hang}` + 每宠 `<id>/warp`），包结构即宿主的 `UserEffect "Pet"`（`SHOW_FOREIGN_EFFECT`：`int cid, byte 4=Pet, byte nType, byte petIndex`）——与宿主 `showOwnPetLevelUp`/`showPetLevelUp` 同形，只是 nType 通用化。本轮实现两个跟随宠物真正会触发的特效：①**Teleport(1)**——`teleportPet`（宠物落后被"过远回位"：remove→改坐标→respawn）在 respawn 同帧广播，即官方的传送 puff；②**HangOnBack(2)**——主人**刚抓上绳/梯**（`ownerWasClimbing` 的上升沿 `grabbedRope = !was && now`）时，在宠物切到 HANG 姿态的同一 tick 广播一次，即官方"宠物跳上主人后背"的 puff。新增 `petEffectPacket(cid, petIndex, nType)` 纯 seam（字节布局可单测，不依赖 Character/MapleMap），`broadcastPetEffect` 走观察者副本（bot 为 headless，自己那份无意义）。新增 `BotPetEffectPacketTest`（3 例）钉住两种 nType 的完整字节与 `Pet` 效果类型字节=4。全量 `mvn test` 1061 项通过。

**第十七轮（宠物特效复查修正）**：对第十六轮的两处复查结论落地。①**语义标注**——`nType=1(Teleport)/2(HangOnBack)` 是**推断**而非实测事实：命中的参考服务端（kinoko）只定义了 `PetEffectType` 枚举、却**只发 LevelUp(0)**；客户端复刻（Journey `PetLook`）把传送/挂背建模为宠物**自身的 WARP/HANG 姿态**而非 UserEffect。资源（`PetEff.img/Basic/{Teleport,hang}`）与枚举值确实存在、且包形状/类型字节 4 已对宿主源码验证，但"这两个 nType 对应这两个触发"无实证。注释已从断言改为显式"INFERENCE / VERIFY IN-GAME"，并写明若不生效则回退 WARP/HANG 姿态路线。②**泳图边界 BUG**——原 `grabbedRope` 未排除 swim 分支：泳图里主人抓绳、宠物脚下无地时走 `followSwim`（SWIM 姿态，不处理爬梯），却仍广播 HangOnBack → "有特效、无挂背姿态"。现门控收紧为 `grabbedRope && !swim`，只在宠物确实进入 HANG 姿态的 land 路径触发。全量 `mvn test` 通过。

**第十八轮（上下层平台"宠物倒退走"）**：实机反馈——**bot 在下层平台、宠物在上层平台时，bot 一移动，宠物就"倒退着走"**（朝向与位移相反）。用真实 `MapleMap`（`Unsafe` 构造）+ 生产 `followLand` 复现并做 4000 组随机双平台场景扫描，定位到**同一根因的两处表现**（缺一不可）：

①**"下落跳"在走路途中误触发**。第十五轮的下落分支条件是 `ownerBelow && walkStayedLevel`（owner 在下、这一 tick 走位没把宠物降下去）。但**宠物沿自己平台水平走路**时每 tick 都以"与起点同高"结束——条件恒真——于是走路途中就进了下落分支：把宠物小跳一下（多数又落回同平台）。扫描：走路中误触发 **22098** 次。修复：抽成纯判定 `dropsToOwnerBelow(ownerOnSameSurface, ownerBelow, walkStayedLevel, exitVelocityPxs)`，**只有 `|vx|<=1`（静止）才算"滞留在被遗弃的平台上"**；正在移动的宠物是在**自己**的平台上正常推进，应继续走（到真边缘由 `walk.lostGround()` 用出边速度与朝向自然下落）→ 误触发 **0**。

②**跳跃/下落姿态用了"上一 tick 的过期朝向"**。三处覆写 `PET_JUMP_*`/落地 STAND 姿态的分支（hop-up、air 空中、下落跳/walk-off）都用上一 tick 存下的 `left` 朝向，而非**本 tick 的实际运动方向**。当宠物在**转向的那一 tick** 起跳/踩空（owner 换到另一侧），朝向仍是旧的，于是"朝右的宠物往左跳/往左滑"——正是倒退走。这是残留的主因：只修 ① 仍有 **442** tick 倒退（136 个案例）。修复：新增纯 seam `facesLeftOnMotion(vx, previousFacingLeft)`——**按本 tick 运动方向定朝向**（`|vx|<=1` 的静止下落无自身方向，保留原朝向，避免直上直下的小跳乱翻面），三处姿态计算一律改用它。①②同修后扫描：倒退 **19022 → 0**（案例 2359 → 0），走路中误跳 **22098 → 0**；且"owner 静止在下层、宠物静止在上层"的第十五轮下落跳**仍照常触发**。新增 `BotPetBackwardWalkTest`（7 例，两个纯 seam）。宠物套件 58 项、全量 `mvn test` 通过。
