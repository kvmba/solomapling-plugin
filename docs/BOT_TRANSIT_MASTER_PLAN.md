...
> **另一半：回程也是独立的一条边。** 多数大陆的出入口不是同一个 NPC，
> 到达不等于能回来。详见「附录四」。
...
**密林方向与别处不同，容易接错**。售票员 `1032007` 只卖船票道具 `4031045`
（`cm.gainItem`，不 warp），真正放人进候船室的是他旁边的检票员 `1032008`
（持有船票且 `entry=="true"` 时 `cm.warp(101000301)`）。所以这条路的第一段要接
`1032008`，接 `1032007` 等于把 bot 送到一个永远不会移动任何人的 NPC 面前。

**地铁的废都端同样是两段**。地铁站大厅 `103000100` 只是售票处，候车室是它后面的
`600010004`（坠落城市，无出边）。只接「大厅→新叶城」一段会让 bot 跳过候车和乘车，
到达时像是根本没上过车。

**阿尔泰（艾琳森林）的入口是脚本门**。`222020400` 时间控制室的 `in01` 走
`move_elin.js` → `300000100`，`tm` 为 `999999999`，任何只读地图链接的路由都看不见它，
必须登记进 `BotScriptedWarp`。
`999999999`——是脚本门，不是地图链接。玩家靠售票处里的向导 `2012006` 艺斯选平台，
他 `cm.warp(200000110 + sel*10)` 把人送到对应「港口通道」。少了这五条边，
bot 到了天空就再也出不去，玩具城/神木村/武陵/阿里安特全部连带不可达。

---

## 附录四 · 回程要单独接（去得到 ≠ 回得来）

接完去程后逐个验证返程，**21 个大陆里有 7 个是单向的**——bot 去了就永久滞留。
原因是这些地方的入口与出口本来就不是同一个 NPC，也不是同一个门。

| 大陆 | 去程 | 回程（原先缺失） |
|---|---|---|
| 黄金海岸 | 三地开船 `1002002`/`2010005`/`2040048` | `1081001` 派申 渡你回港 |
| 日本神社 | 旅行社 `9000020` | 同一个 `9000020`，另一分支（记得你从哪来） |
| 嵩山镇 | 旅行社 `9000020` | 同上 |
| 吉隆大都市 | 旅行社 `9000020` | `9201135` 奥黛丽（当地导游） |
| 甘榜村 | 吉隆坡走进去 | `9201135` 奥黛丽 |
| 昭和村 | 由日本 `801030000` 走进去 | 先回日本，再走旅行社 |
| 时间神殿 | 神木 `2082003` 变龙飞去 | **原路飞回**（见下） |

### 时间神殿：怎么来就怎么回去

它是迁徙阶梯的终点（100 级），接不回程等于把 bot 流放。回程与去程完全对称，
两端都是**脚本门**：

```
去: 240000110 --2082003--> 200090500(龙) --门--> 200090510 --in00/templeenter--> 270000100
回: 270000100 --out00/outTemple--> 200090510 --门--> 200090500 --down00/undodraco--> 240000110
```

### 顺带发现：神木码头 `240000110` 也是个死路

它的门全是脚本门（`west00` 走 `dracoout.js` → `240000100`），**没有任何可走的门**。
小屋（Cabin）和龙（Dragon）的乘客都落在这里，所以这个门不通的话，
从天空坐小屋来神木的 bot 同样下不了船。

### 仍不可达（内容残缺，非遗漏）

圣地 / 里恩 —— 登船脚本缺失或为空函数，玩家同样去不了。

**神木村不是「只到一半」**：82 张有怪图里 35 张连不到枢纽，但那 35 张是
未来之城（时间穿越副本，入口 `240070000` 本身就是孤岛）、喷火龙栖息地（BOSS）、
塔拉森林（需任务）、半人马隐身处与暗黑洞穴（无出口的深层）。都不该让 bot 进。

**时间神殿只有「追忆之路」能进**：`270010000` 由 `270000000` 的 `in00` 门直达，
但「后悔之路」`270020000` 与「忘却之路」`270030000` **没有任何 NPC 或脚本指向它们**，
玩家同样进不去。神殿内部 15 张有怪图里可达的只有 1 张，这是内容残缺而非接错边。
